 ...
 * combines a long-period instance of George Marsaglia's Xorshift generators
 * (described in <a
 ...
 * <p>
 * <p>
 * <strong>Warning</strong>: in version 2.2.3 the seeding procedure has been
 * changed, as it now uses a {@link SplitMix64RandomGenerator}.
 * <p>
 * <p>
 ...
 * number generator. Its period is 2<sup>1024</sup>&nbsp;&minus;&nbsp;1, which
 * is more than enough for any massive parallel application (it is actually
 * possible to define analogously a generator with period
 * 2<sup>4096</sup>&nbsp;&minus;&nbsp;1, but its interest is eminently
 * academic).
 *
 ...
 */
public final class XorShift1024StarRandom extends Random {
     ...
     *
     * @param seed a nonzero seed for the generator (if zero, the generator will
     *             be seeded with -1).
     */
     ...
     * makes &ldquo;warmup&rdquo; unnecessary, and makes the possibility of
    ...
    public void setState(final long[] state, final int p)
        ...
        if (state.length != s.length) {
            throw new IllegalArgumentException("The argument array contains "
                    + state.length + " longs instead of " + s.length);
        }
